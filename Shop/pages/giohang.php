		...
		<div class="footer">
			<div class="grid wide">
							...
							if($ok == 2)
					{
						...
						<td data-th="Product"> 
							<div class="row"> 
							<div class="col-sm-2 hidden-xs"><img src="../'.$row['hinhanh'].'" alt="Sản phẩm 1" class="img-responsive" width="100">
							</div> 
						...
						<td data-th="Price">'.number_format($row['giasp']-($row['giasp']* ($row['giamgia']* 0.01)),3).'đ</td> 
						<td data-th="Quantity">
						<input class="form-control text-center" type="number" name="qty['.$row['id'].']" value="'.$_SESSION['cart'][$row['id']].'">
						</td> 
						<td data-th="Subtotal" class="text-center">'.number_format($_SESSION['cart'][$row['id']]*($row['giasp']-($row['giasp']* ($row['giamgia']* 0.01))),3).' đ</td> 
						...
						<tr> 
						';
						$total += $_SESSION['cart'][$row['id']]*($row['giasp']-($row['giasp']* ($row['giamgia']* 0.01))); 
					...
					?>
						...
						<a href='../pages/delcart.php?productid=0'><img src='../img/thungrac.jpg' width='20px' height='20px'>Xóa bỏ giỏ hàng</a>
									</b>";
					}
					else
					{
